...
class CometChatGroupDetails extends React.Component {
	...
	removeVirgilGroupMember = async (group, uid) => {
		if (uid) {
			const eThree = this.context.eThree;
			const existingParticipant = await eThree.findUsers(uid);
	...
	deleteVirgilGroup = async (guid) => {
		const members = await this.fetchGroupMembers(guid);
		if (members && members.length !== 0) {
			const group = await this.getVirgilGroupInstance(guid);
			if (group) {
				for (const member of members) {
					if (member.uid !== this.context.item.owner) {
						await this.removeVirgilGroupMember(group, member.uid);
					}
				}
				const eThree = this.context.eThree;
				await eThree.deleteGroup(guid);
			}
		}
	};
	...
	onDeleteConfirm = async e => {
		...
		const guid = this.context.item.guid;
		// delete eThree group.
		await this.deleteVirgilGroup(guid);
