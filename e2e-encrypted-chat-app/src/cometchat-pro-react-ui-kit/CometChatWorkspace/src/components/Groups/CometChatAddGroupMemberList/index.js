...
class CometChatAddGroupMemberList extends React.Component {
    ...
    getVirgilGroupInstance = async guid => {
        try {
            if (guid) {
                ...
                const ownerCard = await eThree.findUsers(authenticatedUser.id);
    ...
    addMembersVirgilGroup = async (guid, membersList) => {
        try {
            if (guid && membersList && membersList.length !== 0) {
                const eThree = this.context.eThree; 
                const group = await this.getVirgilGroupInstance(guid);
                if (group) {
                    for (const member of membersList) {
                        if (member && member.uid) { 
                            const newParticipant = await eThree.findUsers(member.uid);
    ...
    updateMembers = () => {
        ...
        if(membersList.length) {
            ...
            const membersToAdd = [];
            CometChat.addMembersToGroup(guid, membersList, []).then(async response => {
                const authenticatedUser = JSON.parse(localStorage.getItem('auth'));
                if (authenticatedUser.id === this.context.item.owner) {
                    await this.addMembersVirgilGroup(guid, membersList);
                }
